...
void print_temperature_result(int channel_number, float temperature_result) {
  ...
  Serial.print(temperature_result,3);
}
...
void print_direct_adc_reading(int channel_number, float direct_adc_reading) {
  ...
  Serial.print(direct_adc_reading,6);
}
...
float read_voltage_or_resistance_results(int channel_number) {
	...
	return (voltage_or_resistance_result,3);
}
    ...
    if (input == 'T'){
      for (int j = 0; j < 5; j++){
        for (int i = 0; i < num_measured_channels; i++) {
                ...
                read_direct_adc_results( int(channel_number));
                temperature_result = read_temperature_results( int(channel_number));
	      }
        temp = temp+temperature_result; 
      }
    Serial.println(temp/5, 3);
  }
}
}
